...
class Homepage extends Component {
    ...
    render() {
        const createModal = 'createContactModal';
        const updateModal = 'updateContactModal';
        ...
        const createContactForm = (
            <div>
                <ContactForm onSubmit={(values) => { createContact(values); this.closeModal(createModal); }} deleteInfo={false} />
            </div>
        );
        const updateContactForm = (
            <div>
                <ContactForm onSubmit={(values) => { updateContact(contactInfo, values); this.closeModal(updateModal); }} info={contactInfo} deleteInfo={false} />
            </div>
        );
        const deleteContactForm = (
            <div>
                <ContactForm onSubmit={() => { deleteContact(contactInfo); this.closeModal(deleteModal); }} deleteInfo={true} info={contactInfo} />
            </div>
        );

        return (
            <div className='page-content'>
                <h1>Washington Post Address Book</h1>
                ...
                {createContactModal
                    ?
                    <Modal close={() => this.closeModal(createModal)}
                    ...
                    : ''}
                {updateContactModal ?
                    <Modal close={() => this.closeModal(updateModal)}
                        title={`Edit ${contactInfo.name}`}
                    >
                        {updateContactForm}
                    </Modal> : ''}
                {deleteContactModal ?
                    <Modal close={() => this.closeModal(deleteModal)}
